...
| `tailHandoff`  | the tail's end: a window 60 s inside Monitor's cap, `--once`, and the one stdout line (`tail.window`/`quiet`/`woke`/`closed`/`lost`) naming the next act with its `--since` bookmark; its header is the decision log        | every spell with a tail (8 of 9) | digestify has no stream; astrolabe and grapevine are presence tails: always the Monitor re-arm, no `--once` (mind-mapper takes `--once` since 2026-09-24, its daemon's presence lingering across the gaps); only scriptorium, astrolabe and mind-mapper stamp an epoch, so only their bookmarks carry one |
