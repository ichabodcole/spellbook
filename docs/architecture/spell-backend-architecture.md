...
| **digestify**   | **`review` only**           | ⚠ _not yet ported._ One entry, not called `cli`, and **CLI-shaped by stdout while carrying a DAEMON's arithmetic** (`SKILL_ROOT = join(SCRIPT_DIR, "..")`) — the two are independent and the playbook used to assume they were one. Single-shot: **no event log, no SSE, no discovery pointer, no second client**, so four of the kit's eight wire modules have no subject and the epoch ruling does not arise. Serves `/` as the built HTML **substituted in memory**, and refuses `index.html` by name so the unsubstituted document cannot escape — a defence `serveFromDist` does not carry. Raises with `stderr` prose + `return 2` at fourteen sites (no `die`, no envelope), and publishes **124 / 130 as session OUTCOMES** in SKILL.md, which stay outside `errors.ts`'s taxonomy per D52. Zero rows in `exit-site-inventory` and `terminator-invariant`, and that is correct. No acc config. |
