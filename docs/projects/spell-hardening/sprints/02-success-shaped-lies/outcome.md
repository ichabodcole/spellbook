...
| ~~**The `--` terminator docs gap**~~ | **⛔ RETRACTED 2026-08-07 — FALSE WHEN WRITTEN.** It claimed "0 of 6 `SKILL.md` files mention it." That was true when the release-note draft was written and **closed hours later by `bbc61c2`** (thoth's ward beat), which documented the terminator in all six spells and shipped in v2.0.0. **The lead copied the stale claim out of the draft into this outcome without re-running it** — the artifact-decay failure this sprint spent a day documenting, committed inside its own closing artifact. _Retraction found one real remainder, since fixed: glamour · imago · magpie never said that an eaten `--session` **retargets** to the machine-global pointer rather than failing. Bounty already did._ |
...
| beat                             | status                                                                                                                                            |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SKILL.md` re-read               | ✅ 3 of 4 clean; the one gap it found (the `--` terminator) was **closed in-sprint by `bbc61c2`**, not carried forward — see the retraction above |
