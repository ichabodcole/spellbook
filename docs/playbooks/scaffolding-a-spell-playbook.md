...
| `type-check-ward.test.ts`               | nothing pinned — zero `tsc --noEmit` errors and every file examined                                                              | your spell's first type error reds it in tsc's own words, and a spell whose files tsc never reaches reds as NOT LOOKED AT             |
