...
| Drive a conjuration through a daemon + thin CLI        | active       | 2026-06-22      | grapevine + imago independently converged; bounty-agent-usable shipped it (first full adopter); scenario: readback-parity (`state` must reach surface parity, not raw fields); reinforced by bounty #40 heartbeat blocked-skip (server `cardOverdue` ⇄ Alpine `staleInfo` mirror kept in lockstep) |
