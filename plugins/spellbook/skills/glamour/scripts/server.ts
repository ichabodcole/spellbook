...
// `src/glamour/backend/server.ts` and ships BUILT at `../dist/server.js`
// (backend convergence Phase 2 — decisions D2, D6 and D12).
//
...
//
// ⚠ AND FOR GLAMOUR THAT PATH HAD TO CHANGE SHAPE, WHICH IS THIS SPELL'S OWN
// SCAR. Astrolabe and magpie already spawned `join(SCRIPT_DIR, "..", "scripts",
// "server.ts")` — up and back down — so their launcher was free. glamour's CLI
// spawned `join(SCRIPT_DIR, "server.ts")`, its own directory, which was correct
// only while the CLI itself lived in `scripts/`. Bundled into `dist/`, that
// resolves to `dist/server.ts`, a file that does not and must not exist: the
// daemon would never start and `open` would report a start timeout. The backend
// source now resolves this file up-and-back-down. `grimoire/spawn-path-ward.test.ts`
// is the instrument that catches a regression here, and it did catch this one.
//
