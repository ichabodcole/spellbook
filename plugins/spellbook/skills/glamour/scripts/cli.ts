...
// `src/glamour/backend/cli.ts` and ships BUILT at `../dist/cli.js`
// (seams Contract 4's built-backend amendment; backend convergence Phase 2).
//
...
// caller types. Keeping a real `.ts` here is what makes the location ruling free
// instead of a roster-wide prose edit.
//
...
// well-formed-looking JSON that stops mid-string. glamour ships large stdout
// payloads (`state --full`), so this is not theoretical for this spell. The full
// account is in the backend source's own entry block.
//
