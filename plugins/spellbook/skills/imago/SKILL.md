...
>
> **⚠ If the eaten flag is `--session`, imago does not fail — it retargets.** It
> falls back to the machine-global `imago-latest.json` pointer, so the command
> lands on **the most recently opened imago session on this machine**, which may
> not be yours.
...
| `batch [--kind generate\|edit] [--prompt ..] [--tag ..] [--edited-from <vid>] [--summary ..] [--models m1,m2,..] <src…>` | Post a produced batch; each `src` = http url, `data:` url, or file path (inlined). `--models` is a comma-separated list that labels each variant **in order, positional to the `src` list** — the first model name goes to the first `src`, and a short list simply leaves the remaining variants unlabelled. |
| `focus <batchId> <variantId>`                                                                                            | Put a variant on the canvas                                                                                                                                                                                                                                                                                   |
| `context <kind> <name…> [--content "<text>"] [--image <path\|url>] [--link active\|quickPrompts] [--tags a,b,c]`         | Add or upsert a Context Library entry. `kind`: `"prompt"`, `"style"`, `"skill"`, or `"context"`. Styles upsert on normalized name (case-insensitive). `--link` attaches the entry into a set: `"active"` (active-context tray) or `"quickPrompts"` (composer quick-prompts).                                  |
...
(below). When you re-arm Monitor with the launcher and that line's `command`,
append the same grep.
...
- **`context.capture`** — the user clicked "capture style" on the focused image.
  Analyze the focused image, then respond with the `context` verb:
  `context style "<name>" --content "<the look>" --image "<focused variant path>" --link active`.
