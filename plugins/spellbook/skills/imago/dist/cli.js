...
` + "  src = an http(s) url, a data: url, or a file path; --models labels each variant in order");
      ...
      for (const [i, p] of pos.entries()) {
        const v = { src: await resolveSrc(p) };
        if (models[i])
          v.model = models[i];
        variants.push(v);
      }
