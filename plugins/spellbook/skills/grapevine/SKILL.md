...
>   reversible) and _Delete…_ (the same confirm as the 🗑).
...
- **Channel lifecycle, from the surface (V2.1).** Right-click a channel in the
  ...
  _Archive_ / _Unarchive_ and _Delete…_; `+` in the rail header creates a
...
Closing a channel from the trash icon or the context menu's _Delete…_ is
