...
`bun …cli.ts … send` invocation: nothing is posted, you get a `…looks like a
leaked grapevine invocation…` error on stderr and exit `2`. In the rare case the
text genuinely contains that command (e.g. documenting it), pass `--force`.
