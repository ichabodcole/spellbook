...
for a verb is rejected at exit 2 with that verb's set enumerated
(`recognized flags: …`). A bare `cli.ts` invocation is a usage error (exit 2,
commands enumerated on stderr), not a help screen — `help`/`--help` remain the
help path at exit 0. The full surface is emitted by `cli.ts schema` (see the
verb table).
...
`open` that would fix it. The refusal's `hint` field is the **verb invocation**
