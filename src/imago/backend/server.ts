...
/**
 * The daemon's ONE entry, and it is the LAUNCHER's to call (D12, playbook B3).
 *
 * ⛔ NO `import.meta.main` BLOCK, FOR TWO REASONS AND EITHER ONE IS FATAL.
 * First, `dist/server.js` is IMPORTED by `<skill>/scripts/server.ts`, so
 * `import.meta.main` is FALSE and the block would never run: the daemon would
 * boot, bind nothing, exit 0, and every integration test would fail as "never
 * answered", which reads like flake. Second — and this is why the daemon keeps
 * no SECOND entry even for convenience — `SKILL_ROOT` is `import.meta.dir/..`,
 * which is the skill root ONLY from `dist/`. Run from `src/imago/backend/` it
 * computes `src/imago/`, finds no `dist/index.html`, silently chooses DEV, and
 * then fails the dev import from the wrong anchor. Offering that entry would be
 * offering a wrong daemon.
 *
 * ⚠ THE TERMINAL `process.exit(exitCode)` MOVED TO THE LAUNCHER VERBATIM and
 * must stay there: it is family E-terminal in `grimoire/exit-site-inventory`,
 * pinned at `<skill>/scripts/server.ts`. A daemon is not a CLI — its teardown
 * has already run inside `main`, which awaits its own drain — so the CLI's
 * `process.exitCode`-and-return rule does NOT apply here, and the two launchers
 * differing on this one line is deliberate. Do not tidy them into a match.
 */
...
export { defaultState } from "../../../plugins/spellbook/skills/imago/shared/types";
