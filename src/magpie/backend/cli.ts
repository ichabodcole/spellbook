...
// link; not "unstyled at 200" — that sentence was never run; magpie's own failure
...
async function cmdOpen(flags: Record<string, string | boolean>) {
  ...
  const proc = spawn(process.execPath, args, {
    ...
    env: process.env,
    // Contract 5 — see daemonCwd(). A wrong cwd skips the Tailwind plugin; on
    // glamour that fails the page outright (500). Assert the invariant, not the
    // status: the utility never reaches the browser when cwd is wrong.
    cwd: daemonCwd(),
  });
  proc.unref();
