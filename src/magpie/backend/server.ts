...
//
// ⛔ `SCRIPT_DIR` HERE IS THE DIRECTORY OF THE EMITTED BUNDLE, NOT OF THIS FILE.
// This module is AUTHORED at `src/magpie/backend/server.ts` and EXECUTES as
// `plugins/spellbook/skills/magpie/dist/server.js` (Phase 1b). `dist/` sits at
// the SAME DEPTH as the `scripts/` it replaced, so every ANCESTOR-relative path
// below is unchanged by the move. A SIBLING-relative one is NOT — see
// `backend.ts`'s `remove.py`, which is where exactly that went wrong and
// shipped. Asserted in `server.test.ts`, not reasoned about.
...
async function main(argv: string[]): Promise<number> {
  ...
  // passes `external` for the surface-HTML glob, so the bundler does not follow
  // this import and leaves the string in `dist/server.js` BYTE-FOR-BYTE. The
  // five `..` therefore count up from
  // `plugins/spellbook/skills/magpie/dist/` — dist → magpie → skills →
  // spellbook → plugins → repo root — and NOT from `src/magpie/backend/`, where
  // the same string would climb out of the repo. Reading it as a normal
...
export type { MagpieState } from "../../../plugins/spellbook/skills/magpie/shared/types";
export { defaultState } from "../../../plugins/spellbook/skills/magpie/shared/types";
