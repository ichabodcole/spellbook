 ...
 * `plugins/spellbook/skills/bounty/scripts/join.ts`.
 *
 * ⛔ THERE IS NO `if (import.meta.main)` BLOCK HERE. This module ships BUNDLED
 * at `../dist/join.js` and is IMPORTED by the launcher, so `import.meta.main`
 * is FALSE in the artifact and a guarded block would never run — the joining
 * agent would get no output and exit 0 (playbook Phase B, B3).
 *
 * ⛔ ⚠ AND THE LAUNCHER FOR THIS ENTRY IS THE **DAEMON** SHAPE
 * (`process.exit(exitCode)`), NOT THE CLI SHAPE — WHICH CONTRADICTS THE
 * PLAYBOOK'S OWN EXAMPLE. Phase B's B2 says "pick by what the entry IS, not by
 * what it is called: bounty's `join.ts` is a CLI shape". By stdout contract it
 * is: it writes JSON lines a caller parses, and the write most at risk is the
 * terminal `disconnected` frame emitted on the line before `main` returns. But
 * the CLI shape was MEASURED HERE AND IT HANGS: `process.exitCode` + a natural
 * return leaves `join.ts > idle timeout reports reason 'timeout'` running to a
 * 15 s test timeout, because a natural exit waits for the loop to drain and
 * this file's WebSocket is not guaranteed closed on every exit path. The
 * `process.exit` was doing DOUBLE DUTY: draining the payload was broken, but
 * force-terminating a live socket was load-bearing. Shipping a hang to fix a
 * truncation is a bad trade.
 *
 ...
 * socket on every path, then return naturally), which is a behaviour change and
 * belongs in its own branch, not smuggled into a relocation whose contract is
 * "nothing the caller sees moves". The exit stays family E-terminal in
 * `grimoire/exit-site-inventory.test.ts`, at the launcher, at the same address
 * it has always had.
 */
