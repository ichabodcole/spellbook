...
// PORT CLOSED. Until 2026-09-09 this line read `join(SCRIPT_DIR, "server.ts")`:
// the CLI's own directory, which was true for exactly as long as the CLI and
// the daemon shared `scripts/`. Bundled into `dist/`, that resolves to
// `dist/server.ts`, a file that does not and must not exist. **The symptom is
// not a crash** — `open` waits out its handshake deadline and reports a start
// timeout, which reads like a slow first bundle build. It is glamour's exact
// shipped defect, and `docs/playbooks/porting-a-spell-playbook.md` B4 predicted
// it here before this file was touched. The up-and-back-down form is right from
// BOTH addresses (`scripts/` and `dist/`), which is why it is the house form.
// `grimoire/spawn-path-ward.test.ts` is the instrument that catches a
// regression, resolving this arithmetic the way the RUNTIME will.
 ...
 * `plugins/spellbook/skills/bounty/scripts/cli.ts`.
 *
 * ⛔ THERE IS NO `if (import.meta.main)` BLOCK HERE, AND ITS ABSENCE IS THE
 * POINT. This module ships BUNDLED at `../dist/cli.js` and is IMPORTED by the
 * launcher, never executed as the process entry — so `import.meta.main` is
 * FALSE in the artifact and a guarded block would never run. The symptom is not
 * a crash: every verb would print nothing and exit 0 (playbook Phase B, B3).
 *
 * ⚠ THE EXIT DISCIPLINE MOVED WITH IT AND MUST NOT BE "TIDIED". This function
 * RETURNS a code; the launcher assigns `process.exitCode` and returns
 * naturally, NEVER `process.exit(code)`. Bun's stdout is asynchronous on a PIPE
 * and synchronous on a TTY or file, so an explicit exit discards whatever has
 * not drained — measured in this repo at exactly 65,536 bytes, with the file
 * form of the same payload complete. The symptom is a truncated JSON body that
 * fails to parse, and the harm is worse than a crash: a reader concluded "our
 * board is too big to read" and three agents worked under that false rule.
 * bounty's `state` on a large board is exactly that payload.
 *
 * `run()` takes NO ARGUMENTS: the command line belongs to the file that PARSES
 * it, which is this one. A launcher that read the argument vector would match
 * the roster enumerator's arg-parsing predicate in
 * `grimoire/lib/entry-points.ts`, and the flag ward would then judge bounty's
