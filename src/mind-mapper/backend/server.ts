...
// ⛔ COMPUTED FROM THE ARTIFACT'S ADDRESS, WHICH IS
// `plugins/spellbook/skills/mind-mapper/dist/server.js` — not from this source
// file. `dist/` sits at the same depth under the skill root as the `scripts/` it
// replaced, so the climb is unchanged by the port; that is a COINCIDENCE OF
// DEPTH and `grimoire/spawn-path-ward.test.ts` is what asserts it rather than
// assuming it (playbook B4/B5).
...
/**
 * The daemon's one entry, called by the launcher at
 * `plugins/spellbook/skills/mind-mapper/scripts/server.ts`.
 *
 * ⛔ THERE IS NO `import.meta.main` BLOCK, AND THAT IS THE POINT.
 * `dist/server.js` is IMPORTED by the launcher, never executed as the process
 * entry, so `import.meta.main` is FALSE in the bundle: a block here would never
 * run, the daemon would boot, serve nothing and exit 0, and every test would
 * fail as "never bound a port" — which reads like flake (playbook B3). The
 * source keeps no second entry either: `SKILL_ROOT` is the skill root only from
 * `dist/`, so run from `src/mind-mapper/backend/` this file computes
 * `src/mind-mapper/`, finds no `dist/index.html`, chooses DEV, and then fails
 * the dev import from the wrong anchor.
 *
 * ⚠ THE TERMINAL `process.exit` STAYS AT THE LAUNCHER, not here: `main` awaits
 * a signal-resolved promise and then runs its own teardown, so the event loop is
 * empty when it returns and either launcher shape works. The exit is family
 * E-terminal in `grimoire/exit-site-inventory.test.ts`, pinned at the launcher,
 * which is now the site where this process ends.
 */
export async function run(): Promise<number> {
  return await main(process.argv.slice(2));
}

// ⛔ THE DAEMON IS A LIBRARY WITH AN ENTRY POINT, AND THIS LINE IS WHY THE
// PORT'S TEST TRIAGE HAS THREE CATEGORIES RATHER THAN TWO. `sse-keepalive.test.ts`
// imports `sseResponse` and drives it IN-PROCESS — it spawns nothing — so that
// reference follows the SOURCE while `server.test.ts`, `lifecycle.test.ts`,
// `presence.test.ts` and `release-serve.test.ts` spawn a PROCESS and follow the
// LAUNCHER. Pointing an importer at the launcher gets a file that exports
// nothing; pointing a spawner at this file boots nothing at exit 0. Neither
// fails as a wrong path (playbook B6.1).
