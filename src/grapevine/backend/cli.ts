...
// ⛔ UP AND BACK DOWN, NEVER A FLAT SIBLING (playbook B4). This read
// `join(SCRIPT_DIR, "daemon.ts")` — glamour's exact shipped defect — which was
// true for exactly as long as the CLI and the daemon shared a folder. From
// `dist/` that resolves to `dist/daemon.ts`, a file that does not and must not
// exist. The symptom is not a crash: the spawn fails silently (the daemon's
// stdio is ignored), no port file ever appears, and the 3 s poll loop below
// reports `daemon failed to start within 3s` — which is ALSO what a launcher
// that exits a live daemon reports (D69) and ALSO what a dev-mode daemon dying
// at its surface import reports (see `ensureDaemon`). Three defect classes, one
// sentence; this is the first of the three.
// `grimoire/spawn-path-ward.test.ts` resolves this arithmetic the way the
// runtime will, from the EMITTED file's own directory, and asserts the file is
// there.
const DAEMON_SCRIPT = join(SCRIPT_DIR, "..", "scripts", "daemon.ts");
...
// `dist/cli.js` is IMPORTED by `scripts/cli.ts`, never executed as the process
// entry, so the guard would never run and every verb would print nothing and
// exit 0. Nor may this file offer a second entry from its authoring address:
// `SKILL_ROOT`, `DIST_DIR`, `SURFACE_CWD` and `DAEMON_SCRIPT` above are all
// computed from `SCRIPT_DIR` and are correct only from `dist/`.
//
// The drain contract lives at the launcher now — `process.exitCode` + a natural
// return, never an explicit exit, because Bun's stdout is ASYNCHRONOUS on a
// pipe and `tail` writes JSONL a caller parses. See
// `plugins/spellbook/skills/grapevine/scripts/cli.ts` for the full account.
