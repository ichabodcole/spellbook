...
import { cn } from "cn";
import { CheckIcon, MessageSquarePlusIcon, Trash2Icon, UndoDotIcon } from "lucide-react";
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3">
        {shown.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {shown.map((n) => (
              <Note key={n.id} note={n} onGoTo={onGoTo} onResolve={onResolve} onRemove={onRemove} />
            ))}
          </div>
        )}
        {resolved.length > 0 && (
