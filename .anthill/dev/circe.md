...
The zone tab strip renders only when zones exist (ratified layout ruling), so the FIRST zone is only mintable conversationally or via CLI — flag for drive #3 if Cole wants a from-zero UI affordance.
