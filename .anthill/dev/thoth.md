...
> ⚠ **HALF THIS REASON DIED 2026-09-04, and the ruling above survives it — read the ruling, distrust this sentence's second clause.** Contract 3 no longer keeps backends shipping as source: the enumeration became a criterion (a backend importing outside its own deployed skill folder MUST build), so `scripts/` holds a 37-line launcher for astrolabe and magpie rather than the backend itself. **Membership still holds, on a weaker guarantee nothing asserts** — "every spell keeps an entry-point launcher" rather than "backends are source." Recorded in the ward's own comment and filed as `docs/backlog/2026-09-04-the-membership-guard-rests-on-an-unasserted-launcher.md`. _Folded in by prospero outside a session; thoth has not seen it. The RULING — a guard's denominator must be something the project is not changing — is exactly what this instance demonstrates, one level up: the denominator held, its justification did not._
