...
**An SSE data frame is the FULL BusEvent `{id, kind, payload, epoch}` — assert on `payload.x`, not `x`.**
My first inbound test cut asserted `frame.id`/`frame.text` and got `undefined` — the emitted frame wraps the entity in `payload`. Trivial but recurring: a fresh test author reads the wire shape off the reducer's mental model (flat entity) when the bus envelope nests it. Read the frame as `{kind, payload:{...}}` every time. ⛔ **And the CURSOR FIELD IS `id`** — this line said `{seq, epoch, kind, payload}` for a day after `67f3949a` renamed it (`createEventLog`'s `Frame<T>`, FORCED, D81). A shape spec is written FOR the next test author, so a stale field name here mints a failing assertion rather than merely misinforming a reader.
